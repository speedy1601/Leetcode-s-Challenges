...
| 29 | In `PANDAS` you can use `df['a'].min()` like this inside `query` using `@`. Just a simple `REMINDER` that in `PANDAS, POLARS` if we filter the table after `GROUP BY(of course along with agg())`, means in `POSTGRESQL` we have to apply `HAVING` after `GROUP BY` because we filter on `GROUP BY` result with `HAVING` CLAUSE in POSTGRESQL.|